...
#include <ojoie/Core/DispatchQueue.hpp>
    ...
    virtual void update(float deltaTime) override {
        ...
        if (soundStream) {
            ...
            if (soundStream->getTotalSize() != 0) {
                ...
                AN::DispatchQueue::async(AN::DispatchQueue::Render, [this, percent]{
    ...
    virtual void render(const AN::RenderContext &context) override {
        ...
        if (show_main_window) {
            ...
            if (ImGui::Combo("Set FPS", &selected_fps, canSelectFPS, std::size(canSelectFPS))) {
                AN::DispatchQueue::async(AN::DispatchQueue::Game, [=, index = selected_fps]{
                    int fps;
                    switch (index) {
                        case 0:
                            fps = 60;
            ...
            if (ImGui::SliderFloat("FreMod", &fre, 0.1f, AN::SoundStream::MaxFrequencyRatio())) {
                AN::DispatchQueue::async(AN::DispatchQueue::Game, [=, this]{
            ...
            if (ImGui::Button("Open File")) {
                AN::DispatchQueue::async(AN::DispatchQueue::Main, [this, window = context.window]{
                    ...
                    openPanel.beginSheetModal(window, [this](AN::Application::ModalResponse response, const char *filePath) {
                        if (response == AN::Application::ModalResponse::Ok && filePath) {
                            AN::DispatchQueue::async(AN::DispatchQueue::Game, [this, path = std::u8string((const char8_t *)filePath)] {
                                soundStream->stop();
            ...
            if (ImGui::IsItemDeactivatedAfterEdit()) {
                AN::DispatchQueue::async(AN::DispatchQueue::Game, [per = userChoosePercentage, this]{
...
//                    soundStream->setCurrentPosition(expectedPosition);
                    ...
                    AN::DispatchQueue::async(AN::DispatchQueue::Render, [this] {
