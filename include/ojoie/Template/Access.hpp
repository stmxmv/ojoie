...
struct Accessor<Tag, Member> {
    ...
    void set(_Cls &obj, T &&value) {
        obj.*Member = std::forward<T>(value);
    }
...
template<typename Tag, typename _Cls, typename Accessor = typename TagTrait<Tag>::AccessorType>
void set(_Cls &obj, typename AccessorTrait<Accessor>::ValueType &&value) {
    return Accessor{}.set(obj, std::forward<typename AccessorTrait<Accessor>::ValueType>(value));
}
